...
SCHARDT.py
==========
Test script to run SIFT-AID on the NIST CFReDS SCHARDT.005 hacking case image.
...
# Set a generous timeout (300 seconds) because SCHARDT.005 is 635MB
...
async def main():
    sample_path = current_dir / "vf_datasets/SCHARDT.005"
    if not sample_path.exists():
        print(f"[-] Error: Dataset file not found at {sample_path}")
        print("Please run 'wget https://cfreds-archive.nist.gov/images/hacking-dd/SCHARDT.005' inside 'vf_datasets/' first.")
        sys.exit(1)

    print("=" * 80)
    print("   SIFT-AID — NIST CFReDS Hacking Case Triage (SCHARDT.005)")
    print("=" * 80)
