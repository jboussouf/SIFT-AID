...
================================
Test script to run SIFT-AID on the cfreds_2015_data_leakage_rm#2.E01 forensic image.
...
# Set a generous timeout (300 seconds) because the image is ~243MB
...
async def main():
    sample_path = current_dir / "vf_datasets/cfreds_2015_data_leakage_rm#2.E01"
    if not sample_path.exists():
        print(f"[-] Error: Dataset file not found at {sample_path}")
        sys.exit(1)

    print("=" * 80)
    print("   SIFT-AID — cfreds_2015_data_leakage_rm#2.E01 Case Triage")
    print("=" * 80)
    ...
    try:
        ...
        final_state = await run_triage(
            ...
            incident_id="INC-CFREDS2015LEAK"
        )
